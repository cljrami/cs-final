...
export default function AutocompleteField({ label, icon, placeholder = 'Escribe para buscar...', options, value, onChange, onSelect }: Props) {
  ...
  const filtrar = useCallback((texto: string) => {
    if (!texto.trim()) {
      setSugerencias(options.slice(0, 10));
      return;
    }
    ...
    <div ref={containerRef}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-[#1a1a24] border border-gray-700 rounded-xl py-3 pl-11 pr-10 text-white placeholder-gray-600 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/30 transition-all text-sm"
        />
        {input && (
          <button
            type="button"
        ...
        {show && sugerencias.length > 0 && (
          <div className="absolute z-50 mt-1 bg-[#1a1a24] border border-gray-700 rounded-xl shadow-2xl shadow-black/50 overflow-hidden max-h-64 overflow-y-auto" style={{ width: inputRef.current?.offsetWidth, top: '100%', left: 0 }}>
            {sugerencias.map((opt, index) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all ${
                  index === selectedIndex ? 'bg-red-500/10 text-red-400' : 'text-gray-300 hover:bg-gray-800'
                }`}
              >
        ...
        {show && input && sugerencias.length === 0 && (
          <div className="absolute z-50 mt-1 bg-[#1a1a24] border border-gray-700 rounded-xl shadow-2xl p-4 text-center text-gray-500 text-sm" style={{ width: inputRef.current?.offsetWidth, top: '100%', left: 0 }}>
