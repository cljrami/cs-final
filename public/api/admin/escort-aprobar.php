...
try {
    ...
    // Usa el plan que la escort seleccionó (de su suscripción), o fallback al más barato
    ...
    // Fallback si no tiene suscripción: el plan base activo más barato
    if (!$plan) {
    ...
    if ((int)$pagoCheck->fetchColumn() === 0) {
        $pdo->prepare("
            ...
            VALUES (?, ?, 'plan', ?, 'CLP', 'transferencia', 'completado', 'Aprobación automática por admin', NOW(), NOW())
        ")->execute([
            $id,
            $plan ? $plan['id'] : null,
            $plan ? $plan['precio'] : 0,
