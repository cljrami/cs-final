...
const tipoIcon: Record<string, string> = {
  ...
  verificacion_aprobada: 'fa-shield-alt text-blue-400',
  verificacion_rechazada: 'fa-shield-alt text-red-400',
  vip_rechazado: 'fa-crown text-red-400',
  fotos_actualizadas: 'fa-images text-green-400',
  plan_aprobado: 'fa-credit-card text-green-400',
  plan_rechazado: 'fa-credit-card text-red-400',
  suscripcion_aprobada: 'fa-credit-card text-green-400',
  comprobante_aprobado: 'fa-receipt text-green-400',
...
export default function NotificationBell() {
  ...
  const fetchNotificaciones = async (unreadOnly = false) => {
    ...
    try {
      ...
      const res = await fetch(`/api/escort/notificaciones.php?${params}`, {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="bg-transparent border border-escort-border text-escort-muted w-9 h-9 sm:w-10 sm:h-10 rounded-lg hover:text-escort-text hover:border-escort-muted transition-all duration-200 flex items-center justify-center relative"
        aria-label="Notificaciones"
      >
        <i className="fas fa-bell text-sm sm:text-base"></i>
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center bg-red-500 text-white text-[10px] font-bold rounded-full px-1 border-2 border-escort-card shadow-lg shadow-red-500/30">
      ...
      {open && (
        <div className="absolute right-0 top-12 w-80 md:w-96 bg-escort-card border border-escort-border rounded-xl shadow-2xl shadow-black/40 z-50 overflow-hidden animate-fadeIn">
          {error && (
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-escort-border">
            <h3 className="text-sm font-semibold text-white flex items-center gap-2">
              <i className="fas fa-bell text-escort-muted"></i>
              Notificaciones
              {unreadCount > 0 && (
            ...
            {unreadCount > 0 && (
              <button
                onClick={markAllRead}
                className="text-xs text-escort-muted hover:text-white transition-colors"
              >
                Marcar todas leídas
              </button>
            )}
          ...
          <div className="max-h-[360px] overflow-y-auto">
            {loading && notificaciones.length === 0 ? (
              <div className="flex items-center justify-center py-8">
                <i className="fas fa-spinner fa-spin text-escort-muted text-lg"></i>
              </div>
            ) : notificaciones.length === 0 ? (
              <div className="flex flex-col items-center py-8 text-escort-muted">
