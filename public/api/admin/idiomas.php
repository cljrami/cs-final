...
try {
    ...
    if ($method === 'GET') {
        ...
        $stats = [
            'total' => (int)$pdo->query("SELECT COUNT(*) FROM idiomas")->fetchColumn(),
            'activos' => (int)$pdo->query("SELECT COUNT(*) FROM idiomas WHERE activo = 1")->fetchColumn(),
            'inactivos' => (int)$pdo->query("SELECT COUNT(*) FROM idiomas WHERE activo = 0")->fetchColumn(),
        ];
        ...
        $countSql = "SELECT COUNT(*) FROM idiomas $whereClause";
    ...
    if ($method === 'POST') {
        ...
        if (empty($nombre)) {
            $fieldErrors['nombre'] = 'El nombre del idioma es obligatorio';
        ...
        if (empty($fieldErrors['nombre'])) {
            $checkStmt = $pdo->prepare("SELECT id, nombre FROM idiomas WHERE LOWER(nombre) = LOWER(?)");
            $checkStmt->execute([$nombre]);
            $existing = $checkStmt->fetch(PDO::FETCH_ASSOC);
            if ($existing) {
                $fieldErrors['nombre'] = 'Ya existe un idioma llamado "' . $existing['nombre'] . '"';
            }
        }
        ...
        $stmt = $pdo->prepare("
            INSERT INTO idiomas (nombre, orden, activo) 
            VALUES (?, ?, ?)
        ");
    ...
    if ($method === 'PUT') {
        ...
        $checkStmt = $pdo->prepare("SELECT id FROM idiomas WHERE id = ?");
        ...
        if (isset($input['nombre'])) {
            ...
            if (empty($nombre)) {
                ...
                $dupStmt = $pdo->prepare("SELECT id, nombre FROM idiomas WHERE LOWER(nombre) = LOWER(?) AND id != ?");
                $dupStmt->execute([$nombre, $id]);
                $existing = $dupStmt->fetch(PDO::FETCH_ASSOC);
                if ($existing) {
                    $fieldErrors['nombre'] = 'Ya existe otro idioma llamado "' . $existing['nombre'] . '"';
                } else {
                    $updates[] = 'nombre = ?';
                    $values[] = $nombre;
                }
        ...
        $values[] = $id;
        $sql = "UPDATE idiomas SET " . implode(', ', $updates) . " WHERE id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($values);

        $getStmt = $pdo->prepare("SELECT * FROM idiomas WHERE id = ?");
        $getStmt->execute([$id]);
        $idioma = $getStmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($method === 'DELETE') {
        ...
        $checkStmt = $pdo->prepare("SELECT nombre FROM idiomas WHERE id = ?");
        $checkStmt->execute([$id]);
        $idioma = $checkStmt->fetch(PDO::FETCH_ASSOC);
        ...
        // Verificar si hay escorts usando este idioma
        $usageStmt = $pdo->prepare("SELECT COUNT(*) FROM escort_idiomas WHERE idioma_id = ?");
